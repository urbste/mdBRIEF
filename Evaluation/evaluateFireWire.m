...
for im = 1:nrImages
    ...
    fastType = 2; nLevels = 8; doOrientation = 1; desc_sizes = 128;
    % now you can select a test set. see mdBRIEFFireWire.cpp for details
    testSet = 3;
    [keypoints{im}.kp{4},descriptors{im}.Descs{4},descriptors{im}.Mask{1}] = cv.mdBRIEFFireWire(Images{im}, ...
        nrPts, nLevels, learnMasks, fastType, useAgast,...
        desc_sizes, doOrientation, testSet);
    
    [keypoints{im}.kp{6},descriptors{im}.Descs{6},descriptors{im}.Mask{3}] = cv.mdBRIEFFireWire(Images{im}, ...
        nrPts, nLevels, learnMasks, fastType, useAgast,...
...
%                 showMatchedFeatures(Images{idx1},Images{idx2},pts1(:,indices_match1(M))',pts2(:,indices_match2(M))')
