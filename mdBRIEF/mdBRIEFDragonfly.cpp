	...
	*/
		...
		bool useProvidedKeypoints)
	{
		...
		if (do_descriptors)
		{
            ...
            switch (testSet)
            {
                ...
                case 2: pattern0 = (const Point*)learned_akaze_pattern_64_perspective; break;
                case 3: pattern0 = (const Point*)learned_akaze_pattern_64_fisheye; break;
                case 4: pattern0 = (const Point*)learned_bold_pattern_64_; break;
                case 5: pattern0 = (const Point*)brief_pattern_64_; break;
			...
			compute_dBRIEF_descriptor(imagePyramid, layerInfo, layerScale,
				...
				pattern, dsize, camModel, learnMasks,doRotation);
		}
		...
		cCamModelGeneral& camModel)
	{
		...
		if (do_descriptors)
		{
			...
			const int npoints = 2 * 8 * dsize; //2*256
			//Point patternbuf[npoints];
            const Point* pattern0;
            switch (testSet)
            {
                  ...
                  int nrhs, const mxArray *prhs[] )
{
    ...
    cv::Vec<double, 5> cdeu0v0(0.998740725345746,
		-0.269588951600402,
		0.26927809670202,
		366.5771683202,
		227.700678588997);
	cv::Mat p(4, 1, CV_64FC1);
	p.at<double>(0) = -510.270889759497;
	p.at<double>(1) = 0.0;
	p.at<double>(2) = 0.000757390044673718;
	p.at<double>(3) = 5.19399295691985e-07;
	cv::Mat invP(7, 1, CV_64FC1);
	invP.at<double>(6) = -37.6699148800904;
	invP.at<double>(5) = -223.941568880232;
	invP.at<double>(4) = -493.093437992936;
	invP.at<double>(3) = -503.362707415818;
	invP.at<double>(2) = -313.734547937157;
	invP.at<double>(1) = 261.669014796744;
	invP.at<double>(0) = 660.484754532667;
    ...
    mdBRIEF_Impl test_mdBrief(nFeats, scaleFactor, nLevels, edgeThreshold,
            ...
            fastType, learnMask,descSize, doRotation, testSet,1);
